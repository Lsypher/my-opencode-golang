...
1. **Run the package-manager detector** (ships with opencode):

   ```bash
   node scripts/setup-package-manager.js --detect
   ```

   It resolves the package manager (npm / pnpm / yarn / bun) from, in order: `OPENCODE_PACKAGE_MANAGER`, `.opencode/package-manager.json`, the `package.json` `packageManager` field, the lockfile, then global config.
...
.opencode/tdd/<plan-or-task-name>.tdd.md
```

If the repository already uses opencode-specific local artifacts, the `.opencode/tdd/` location is also acceptable. Include:
